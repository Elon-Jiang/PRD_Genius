...
export const refineBreakdown = async (
  ...
  const context = JSON.stringify({ features: currentFeatures, userStories: currentUserStories });
        ...
        properties: {
          ...
          competitors: {
            ...
            items: {
              ...
              required: ['id', 'name', 'coreAdvantage', 'mainDisadvantage', 'overlap', 'ourAdvantage']
            }
          }
        },
...
export const generateMindMap = async (productName: string, features: Feature[], userStories: UserStory[]): Promise<string> => {
  ...
  const storyList = userStories.map(s => `- ${s.role}: ${s.action}`).join('\n');
...
export const sendChatMessage = async (history: {role: 'user' | 'ai', content: string}[], message: string): Promise<string> => {
  const chat = ai.chats.create({
    model: modelFlash,
    config: {
      systemInstruction: "You are a helpful Product Manager Assistant. You can help answer questions about product design, market research, technical feasibility, or general inquiries. Answer concisely and in Chinese (Simplified)."
    },
